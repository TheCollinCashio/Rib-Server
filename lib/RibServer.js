...
class RibServer {
    ...
    **/
    ...
    setCustomHook() {
        this._nameSpace.adapter.customHook = ({ key, args, query, isInclude }, cb) => {
            this._socketMap.forEach(socket => {
                let persistentObj = this.getPersistentObject(socket);
                if (Helper_1.doesObjectMatchQuery(persistentObj, query) && isInclude) {
                    let fn = persistentObj[key];
                    if (typeof fn === 'function') {
                        fn(...args);
                    }
                }
                else if (!Helper_1.doesObjectMatchQuery(persistentObj, query) && !isInclude) {
                    let fn = persistentObj[key];
                    if (typeof fn === 'function') {
                        fn(...args);
                    }
